    ...
    const char **error_reason)
{
	...
	if (evbuffer_get_length(postbuf) > 0) {
		...
		if ((var = evhttp_find_header(&hdr_qs, "init")) != NULL) {
			if (string_to_i32(&init, var)) {
				LOG(LOG_ERR, "failed in convert to int from string");
				init = 0;
			}
		}
		free(tmp);
	...
	if (cmd_type == EVHTTP_REQ_GET && strcmp(&decoded_path[api_url_path_len], API_DEVICIES_URL) == 0) {
		...
		evbuffer_add(response, "[", 1);
		if (fplug_device_hourly_power_total_foreach(http_server->fplug_device, address, &end_tm, init, create_hourly_power_total_response, &api_callback_arg)) {
		...
		evbuffer_add(response, "[", 1);
		if (fplug_device_hourly_other_foreach(http_server->fplug_device, address, &end_tm, create_hourly_other_response, &api_callback_arg)) {
			LOG(LOG_ERR, "failed in hourly power total foreach");
			*error_status_code = HTTP_INTERNAL;
			*error_reason = "failed in hourly power total foreach";
			error = 1;
    ...
    void *cb_arg)
{
	...
	len = snprintf(buf, sizeof(buf),
            ...
            idx, temperature, humidity, illuminance);
	evbuffer_add(api_callback_arg->response, buf, len);
	api_callback_arg->idx++;
}
